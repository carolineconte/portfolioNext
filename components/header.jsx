  ...
  return (
    <header className='z-[999]'>
      ...
      <nav className=' fixed top-[1rem] left-1/2 -translate-x-1/2 flex-wrap py-2 sm:top-[1.7rem] sm:h-[initial] sm:py-0'>
        ...
        sm:w-[initial] sm:flex-nowrap sm:gap-5 dark:bg-gray-950' >
          {links.map((nameSection, i) => (
            <li key={nameSection} className='h-3/4 flex items-center justify-center'>
              <Link href={`#${links[i]}`}
                className={`${activeSection === nameSection ? 'bg-black bg-opacity-15' : ''}
                flex w-full relative items-center justify-center px-5 py-2 rounded-full
                hover:text-gray-950 transition dark:text-gray-500 dark:hover:text-gray-200`}
